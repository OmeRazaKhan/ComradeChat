    ...
    html.Div([
        html.Div([
            ...
            html.Div(id='conversation',
                     style={'max-height': '500px', 'overflow-x': 'hidden', 'overflow-y': 'scroll', 'display': 'flex', 'flex-direction': 'column-reverse'}),
            html.Br(),
            html.Table([
                html.Tr([
                    # user text input
...
def process_message(n_clicks, history, text):
    ...
    user_msg = [html.P(user, style={'text-align': 'right', "margin" : "1px"}) for user in user_text]
    response_msg = [html.P(html.I(res), style={'text-align': 'left', "margin" : "1px"}) for res in response_text]

    response_url = ""
    if "title" in temp_response and "dataset_url" in temp_response:
        response_url = [html.P(html.A(temp_response["title"], href=temp_response["dataset_url"]), style={'text-align': 'left', "margin" : "1px"})]
    
    if history:
        # order reversed so new lines always appear at the bottom in combiantion with CSS
        return response_url + response_msg + ["ComradeChat Bot:"] + user_msg + [html.Hr(style={'width': '100%'})] + history, ''
